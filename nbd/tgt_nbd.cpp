		...
		unsigned len, unsigned msg_flags)
{
	...
	io_uring_prep_fused_master(sqe, dev_fd, tag, q_id, 0);

	io_uring_prep_send_zc(slave, fd, (void *)(u64)offset, len, msg_flags, 0);

	/* send_zc doesn't support IOSQE_CQE_SKIP_SUCCESS */
		...
		unsigned done, const struct ublk_io_data *data)
{
	...
	if (!use_zc || reply)
		...
		io_uring_prep_recv_ublk_zc(sqe, 0, data->tag, q->q_id, q->q_id + 1,
			buf, done, len - done);
		...
		const struct io_uring_cqe *cqe)
{
	...
	/* supposed to be send_zc, which doesn't support sqe skip success */
		...
		struct nbd_queue_data *q_data)
{
	...
	if (q_data->last_send_sqe) {
		...
		/* Keep slave request's link in mind */
